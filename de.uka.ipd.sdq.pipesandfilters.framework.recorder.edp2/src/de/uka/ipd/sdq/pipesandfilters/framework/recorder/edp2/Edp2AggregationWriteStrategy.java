 ...
 * 
 */
public class Edp2AggregationWriteStrategy extends AbstractEDP2Recorder implements
IAggregationWriteStrategy {
    ...
    public void initializeAggregatedMeasurements(
            final AggregationMetaDataInit metaData) {

        final FixedWidthAggregatedMeasurements aggregatedMeasurements = ExperimentDataFactory.eINSTANCE
                .createFixedWidthAggregatedMeasurements();

        // Set metric
        aggregatedMeasurements.setMetric(metricDescription);

        // Set aggregated metric
        aggregatedMeasurements.setAggregationOn((BaseMetricDescription) ((MetricSetDescription)metricDescription)
                .getSubsumedMetrics().get(metaData.getAggregatedMetricIndex()));

        // Set aggregation function description
        final AggregationFunctionDescription desc = ExperimentDataFactory.eINSTANCE
    ...
    public void setFixedWidthAggregatedMeasurementsMetaData(
            final int aggregatedMeasurementsIndex,
            final FixedWidthAggregationMetaData metaData) {

        final FixedIntervals intervals = ExperimentDataFactory.eINSTANCE
    ...
    public void flush() {
        ...
        for (final AggregatedMeasurements measurements : measurementsRange
                .getAggregatedMeasurements()) {
            final FixedWidthAggregatedMeasurements fixedMeas = ((FixedWidthAggregatedMeasurements) measurements);
