...
public abstract class Edp2WriteStrategy {
	...
	public void initialize(MetaDataInit metaData) {
		...
		EDP2MetaDataInit edp2MetaData = getEDP2MetaDataInit(metaData);
		EDP2Config edp2Config = getEDP2RecorderConfiguration(edp2MetaData);
	 ...
	 */
	private EDP2MetaDataInit getEDP2MetaDataInit(MetaDataInit metaData) {
	...
	private EDP2Config getEDP2RecorderConfiguration(
		...
		EDP2Config edp2Config = (EDP2Config) edp2MetaData.getRecorderConfiguration();
		return edp2Config;
	}
	 ...
	 */
	private static void initializeRepository(EDP2Config edp2Config) {
	 ...
	 * @param edp2MetaData 
	 */
	private static void initializeExperimentGroup(EDP2MetaDataInit edp2MetaData) {
	 ...
	 */
	private static void initializeExperimentRun(EDP2MetaDataInit edp2MetaData) {		
	 ...
	 */
	private static void initializeMetricSetDescription(EDP2MetaDataInit edp2MetaData) {
		// TODO Lehrig: Add support for UUIDs into ProbeSpec. Then, we wouldn't have to assume unique names here.
		String metricName = edp2MetaData.getMetricName();		
		
		// Find existing description based on metric name (assumed to be unique)
		for(Description d: repository.getDescriptions()) {
		...
		for (MeasurementMetric measuredObject : edp2MetaData.getMeasuredMetrics()) {
			String subsumedMetricName = measuredObject.getName();
	 ...
	 */
	private static void initializeEDP2Measure(EDP2MetaDataInit edp2MetaData) {
		...
		// be set here.
		String measuredObject = edp2MetaData.getMeasurementName();
		
		// Check for existing Edp2Measures in the experimentGroup
		for(Edp2Measure edp2Measure : experimentGroup.getMeasure()) {
	 ...
	 */
	private static void initializeMeasurements(EDP2MetaDataInit edp2MetaData) {
	...
	public void writeData(PipeData pipeData) {
		Measurement measurement = new Measurement(metricSetDescription);
