...
public class ExecutionResultCalculator extends UnaryCalculator {
     ...
     */
    public ExecutionResultCalculator(final ProbeSpecContext ctx, final Probe probe) {
        super(ctx, Calculator.createMetricSetDescription(Arrays.asList(MetricDescriptionConstants.EXECUTION_RESULT_METRIC)), probe);
    }
    ...
    protected Measurement calculate(final List<Measurement> probeSetSamples) {
        ...
        result.add(obtainMeasurement(probeSetSamples.get(0), MetricDescriptionConstants.EXECUTION_RESULT_METRIC));
