...
/**
 * Identity calculators are unary calculators that directly let probe
 * measurements pass through. For example, the current state of a
 * "take passive resource state probe" may directly be passed through in order
 * to determine its utilization.
 * 
...
public class IdentityCalculator extends UnaryCalculator {
	...
	/**
	 * Default constructor. Directly passes all elements to the unary calculator
	 * constructor.
	 * 
	 * @param ctx
	...
	protected Measurement calculate(
		...
		if (measurement instanceof MeasurementTupple) {
			return new MeasurementTupple(
					((MeasurementTupple) measurement).getSubsumedMeasurements(),
					(MetricSetDescription) this.getMetricDesciption());
		} else {
			throw new IllegalStateException(
					"MeasurementTupple expected for identity calculators");
		}
	}
}
